...
int main()
{
    ...
    matrix_randomize(hidden_weights,11);//2x2
    ...
    for(int i = 0; i < epochs; i++)
    {
        ...
        Matrix *hidden_layer_input = dot(hidden_weights,transpose(inputs)); // 2x4
        ...
        Matrix *final_input = dot(transpose(output_weights),hidden_layer_output);//1x4
        //printf("final_input :\n");
        ...
        Matrix* output_errors = substract(transpose(expected_output), final_output);//1x4
        //printf("error :\n");
        //matrix_print(output_errors);
        Matrix* hidden_errors = dot(output_weights,output_errors);//2x4
        //matrix_print(hidden_errors);

        Matrix* sigmoid_primed_mat = sigmoidPrime(final_output);//4x1
        ...
        Matrix* scaled_mat = scale(learning_rate,dot_mat);

        Matrix* added_mat = add(transpose(output_weights), scaled_mat);//2x1 + 2x1 but initially output_weights not transposed (?)
        matrix_free(output_weights);
        output_weights = transpose(added_mat);
        ...
        //transposed_mat = transpose(inputs);
        dot_mat = dot(multiplied_mat,inputs);//2x2
        scaled_mat = scale(learning_rate, dot_mat);
        added_mat = add(hidden_weights, scaled_mat);
